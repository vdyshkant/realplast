...
(function ($) {
  ...
  $.fn.waterwheelCarouselExtended = function (startingOptions) {

    // Adds support for intializing multiple carousels from the same selector group
    if (this.length > 1) {
      this.each(function () {
        $(this).waterwheelCarouselExtended(startingOptions);
      });
      return this; // allow chaining
    }
    ...
    function performCalculations($item, newPosition) {
      ...
      var newWidth = distanceFactor * 110;
      var newHeight = distanceFactor * 152;
      var widthDifference = Math.abs($item.width() - newWidth);
      var heightDifference = Math.abs($item.height() - newHeight);
    ...
    this.reload = function (newOptions) {
      ...
      options = $.extend({}, $.fn.waterwheelCarouselExtended.defaults, newOptions);
  ...
  $.fn.waterwheelCarouselExtended.defaults = {
    // number tweeks to change apperance
    startingItem: 0,   // item to place in the center of the carousel. Set to 0 for auto
    separation: 355, // distance between items in carousel
    separationMultiplier: 0.6, // multipled by separation distance to increase/decrease distance for each additional item
